...
int s21_sscanf(const char* str, const char* format, ...) {
  ...
  for (int i = 0; i < patterns.size && err == 0; i++) {
    ...
    if (!patterns.data[i].isChar && !patterns.data[i].skipping) {
      param = va_arg(dest, void*);
    }
    scanPattern(&str, patterns.data[i], param, &succ_cntr, str_start, &err);
  }
...
void scanPattern(const char** str, struct Pattern pattern, void* dest,
  ...
  if (pattern.isChar) {
    ...
    if (pattern.sym == 'n') {
      *(int*)dest = *str - str_start;
      //(*succ_cntr)++; Нет
      return;
    }

    if (pattern.sym == 'c') {
      *err = scanChar(str, pattern, dest);
      if (!pattern.skipping) {
        *succ_cntr += 1 - *err;
      }
      return;
    ...
    switch (pattern.sym) {  // *
      case '%':
        *err = scanPerc(str, pattern);
        //(*succ_cntr)++; Нет
        break;

      case 'd':
      case 'u':
        *err = scanInt(str, pattern, dest);
        if (!pattern.skipping) {
          *succ_cntr += 1 - *err;
        }
        break;
      ...
      case 'g':
      case 'G':
        *err = scanFloat_Sci(str, pattern, dest);
        if (!pattern.skipping) {
          *succ_cntr += 1 - *err;
        }
        break;
      case 'i':
        *err = scanMultiDec(str, pattern, dest);
        if (!pattern.skipping) {
          *succ_cntr += 1 - *err;
        }
        break;
...
int scanFloat(const char** str, struct Pattern pattern, void* dest) {
  ...
  if (**str == '.' && (pattern.width > len || pattern.width <= 0)) {
    ...
    if (pattern.width != 0) {
      int len1 = s21_strspn(*str, "0123456789");
      if (pattern.width > 0 && pattern.width < len1) {
        len1 = pattern.width;
      }
      forScan.width = pattern.width;
      scanInt(str, forScan, &dec_part);
      res += dec_part * pow(10, -1 * len1);
    }
  }
...
int scanPerc(const char** str, struct Pattern pattern) {
  int len = s21_strspn(*str, "%");
  if (len == 0) {
    return 1;
  }
  if (pattern.width > 0 && pattern.width < len) {
