...
START_TEST(test_s21_insert_test_5) {
  const char *src = "goodbye";
...
START_TEST(test_s21_insert_test_6) {
  const char *src = s21_NULL;
  const char *str = "bebebe";
  size_t n = 4;
  char *result = (char *)s21_insert(src, str, n);
  ck_assert_pstr_eq(result, s21_NULL);
  free(result);
...
Suite *make_s21_insert_suite(void) {
  ...
  tcase_add_test(tc, test_s21_insert_test_5);
  tcase_add_test(tc, test_s21_insert_test_6);
